 ...
 * 2. Types
 *    2.1 evaluate constant expressions (CONST, ARRAY indexes)
 *    2.2 VAR declaration type exists and assignment lhs compatible rhs (lhs is var, lhs and rhs type compat)
...
private:
  ...
  std::vector<std::string> parse_identifier(std::shared_ptr<ParserNode> identifer, std::shared_ptr<SymbolScopeNode> current_scope);
  ...
  std::shared_ptr<Node> parse_type(std::shared_ptr<ParserNode> type, bool var, std::shared_ptr<SymbolScopeNode> parent);
  ...
  std::shared_ptr<DeclarationNode> var_declaration(std::shared_ptr<const ParserNode> var_decl, std::shared_ptr<SymbolScopeNode> scope);
